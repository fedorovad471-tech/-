# Our C++ lessons
*How we work in lessons on C++ programming :*
1. At the beginning of the lesson, we are given a **theory**.
2. Next, together we **discuss** some suggestive lesson tasks.
3. After that, we **solve** the tasks from the contest on our own.
4. And of course, we practice and **solve** tasks **at home**.
      - Write codes
      - Do our contests
   

# A few memes about programming
![alt text](https://cs13.pikabu.ru/post_img/big/2021/01/31/6/161208022211434908.jpg)
![alt text](https://www.androidauthority.com/wp-content/uploads/2016/02/code-meme-16x9-trimmed.jpg)
![alt text](https://cs17.pikabu.ru/s/2025/11/23/16/wm3flnpy_lg.jpg)

**You can find C++ hints** [here](https://cppreference.com/)
| day of the week | favourite subject | mark |
| ------------- |:-------------:| -----:|
| Monday| Informatics | 5 |
| Tuesday | C++ | ? |
| Friday | Algebra | 5 |

Thanks for your attention! :heart:
